...
const ACTIVITY_ID = "team-order";

export function createTeamOrderActivity({ main, modalRoot, navigate }) {
...
function renderList() {
  main.innerHTML = `
    ...
    <section>
      <div class="table-titlebar"><span class="table-title">活动配置列表</span><span style="color:#909399;font-size:12px">共 ${repository.list().length} 条</span></div>
      ...
      <div class="pagination"><span>共 ${repository.list().length} 条</span><select style="width:90px"><option>10条/页</option></select><span class="page-box">‹</span><span class="page-box active">1</span><span class="page-box">›</span><span>前往</span><input style="width:46px;height:28px" value="1"><span>页</span></div>
    </section>`;
  drawActivityRows(repository.list());
  document.querySelector("#queryBtn").onclick = filterActivities;
  document.querySelector("#resetBtn").onclick = () => { ["qCode","qName","qStatus"].forEach(id => document.querySelector(`#${id}`).value = ""); drawActivityRows(repository.list()); };
...
function filterActivities() {
  ...
  drawActivityRows(repository.list().filter(x => (!code || x.code.toLowerCase().includes(code)) && (!name || x.name.toLowerCase().includes(name)) && (status === "" || String(x.status) === status)));
}

function openEdit(code, readonly) {
  state.editing = code ? repository.find(code) : null;
...
function renderEdit() {
  ...
  ["backList","backList2","closeBtn"].forEach(id => document.querySelector(`#${id}`).onclick = () => go("list"));
  if (!state.readonly) {
...
function saveActivity() {
  ...
  const overlappingActivity = status === 1 && begin && end && begin < end
    ? repository.list().find(activity => activity.status === 1
  ...
  if (state.editing) {
    repository.update(state.editing.code, { name, begin, end, status, tiers:tierConfig, ...frontStyleConfig, modified:nowText(), modifier:"当前用户" });
  } else {
    repository.create({ code:`DUO-202609-${String(repository.list().length+3).padStart(3,"0")}`, name, begin, end, status, tiers:tierConfig, ...frontStyleConfig, modified:nowText(), modifier:"当前用户" });
  }
  toast("保存成功");
...
function confirmReissue(rewardId) {
  ...
  return {
    ...
    menuItems: [
      {
        title: "组队完单活动配置",
        route: `${ACTIVITY_ID}/list`,
        activeRoutes: [`${ACTIVITY_ID}/list`, `${ACTIVITY_ID}/edit`],
      },
      {
        title: "组队完单数据报表",
        route: `${ACTIVITY_ID}/data`,
        activeRoutes: [`${ACTIVITY_ID}/data`],
      },
    ],
    routes: {
