...
function saveActivity() {
  ...
  if (state.editing) {
    repository.update(state.editing.code, { name, begin, end, status, ...frontStyleConfig, modified:nowText(), modifier:"当前用户" });
  } else {
    repository.create({ code:`DUO-202609-${String(repository.list().length+3).padStart(3,"0")}`, name, begin, end, status, ...frontStyleConfig, modified:nowText(), modifier:"当前用户" });
  }
  toast("保存成功");
